  ...
  <v-layout align-center justify-center style="z-index: 2">
    <v-flex xs12 sm6 md4>
        ...
        <v-card-title primary-title class="text-xs-center blue white--text" id="register-card-title">
          <transition name="delayed-fade" mode="out-in">
            <div key="registration" v-if="!registered" style="width: 100%">
              <h1 class="headline">{{ $t('registration') }}</h1>
            ...
            <div key="success" v-else style="width: 100%">
              <h1 class="headline">{{ $t('thank-you') }}</h1>
        ...
        <transition name="shrink" duration="500">
          <v-card-text v-show="!registered" v-model="valid">
            <v-form @submit.prevent="submit" ref="form">
              <v-text-field light autofocus :label="$t('email')" name="email" ref="email" type="email" tabindex="1" required autofocus
              ...
              <v-text-field light :label="$t('password')" name="password1" ref="password1" tabindex="2" required
                            v-model="password1"
                            prepend-icon="lock"
                            v-validate="'required'"
              ...
              <v-text-field light :label="$t('confirm-password')" name="password2" ref="password2" tabindex="3" required
                            v-model="password2"
              ...
              <vue-recaptcha ref="invisibleRecaptcha"
                             ...
                             :sitekey="sitekey"
                             tabindex="-1">
                <v-btn light block color="blue" class="white--text" :disabled="errors.any() || isNotValidated()" type="submit" :loading="working" tabindex="4">{{ $t('register') }}</v-btn>
              </vue-recaptcha>
            </v-form>
          </v-card-text>
        </transition>
      ...
      <transition name="fade">
        <v-btn v-show="!registered" flat block class="white--text mt-3" :to="{ name: 'user:login' }">{{ $t('already-registered') }}</v-btn>
      </transition>
