  ...
  <v-card class="elevation-0">
      ...
      <transition name="fade" mode="out-in">
        <v-layout row wrap v-if="ready" key="map">
          <v-flex xs4 class="pb-0">
            <img @click="onMapClick" :src="mapUrl" :alt="$t('profile.session.approximate-location')" border="0" style="width: 100%; border-radius: 3px" :class=" bogon ? 'not-clickable' : 'clickable'"/>
          </v-flex>
    ...
    <transition name="fade" mode="out-in">
      <v-card-actions class="px-0 pt-0" v-if="ready">
        <v-btn block flat color="red" class="mx-0" :disabled="session.current === true" @click="$emit('end')">{{ $t('label.end-session') }}</v-btn>
      </v-card-actions>
    </transition>
  </v-card>
</template>
